...
### 20. 🔄 Пакетный трансформатор смещения точек (BASE vs WORLD)
*Групповое смещение координат точек с хирургической точностью.*
* **Двойная система координат**: смещение относительно системы координат детали (**BASE**) либо глобальной декартовой системы робота (**WORLD**) через матрицу обратного вращения Эйлера ($R^T_{base} \cdot \vec{\Delta}_{world}$).
* **3D изометрическая векторная схема (SVG)**: интерактивная схема осей с динамическим вектором смещения.
* **Выборочное смещение**: фильтрация точек, добавление пользовательских точек, предварительный просмотр с подсветкой дельт и атомарная запись в `.dat` с сохранением Undo.

---

### 21. 🌓 100% Адаптивные темы и полная шестиязычная локализация
...
* **Переключатель 6 языков**: динамическое переключение языка интерфейса (RU, EN, DE, ES, IT, TR) прямо в шапке инструментов.
...
| **Пакетный трансформатор смещения точек** (BASE / WORLD) | ❌ | **✅ Pro** | 3D схема осей, матрица Эйлера и точечная правка координат `.dat` |
...
* 🏭 **Enterprise для завода (Enterprise Site — $1,499/год):**
  ...
  * **Прямая инженерная поддержка:** приоритетная связь напрямую с разработчиком в Telegram (@kukakrlbot).
