...
export class HighlightProvider {
  public onDocumentHighlight(
    ...
    // Find if current position is on a word or system variable
    const wordRegex = /(\$?\w+)/g;
    let match;
    let keyword = "";
    let keywordStart = -1;
    ...
    // Feature: Highlight all occurrences of the same variable/symbol in the document
    const allText = document.getText();
    const symbolRegex = new RegExp(
      actualWord.startsWith("$")
        ? `(?:^|[^\\w$])${actualWord.replace(/\$/g, "\\$")}\\b`
        : `\\b${actualWord}\\b`,
      "gi",
    );
    ...
    while ((symbolMatch = symbolRegex.exec(allText)) !== null) {
      ...
      highlights.push({
        range: {
          ...
          end: document.positionAt(matchIdx + actualWord.length),
        },
        kind: DocumentHighlightKind.Read,
      });
    }

    // Existing logic for block pairs (IF/ENDIF etc.)
    if (BLOCK_PAIRS[keyword]) {
      const closingKeyword = BLOCK_PAIRS[keyword];
      const closingLine = this.findClosingLine(
        lines,
        pos.line,
        keyword,
        closingKeyword,
      ...
      if (closingLine !== -1) {
        ...
        const closingIndex = lines[closingLine]
          .toUpperCase()
          .indexOf(closingKeyword);
        highlights.push({
          range: {
            start: { line: closingLine, character: closingIndex },
            end: {
              line: closingLine,
              character: closingIndex + closingKeyword.length,
            },
          },
          kind: DocumentHighlightKind.Read,
        });
    ...
    // If it's a closing keyword
      ...
      const openingLine = this.findOpeningLine(
        lines,
        pos.line,
        keyword,
        openingKeyword,
      ...
      if (openingLine !== -1) {
        ...
        const openingIndex = lines[openingLine]
          .toUpperCase()
          .indexOf(openingKeyword);
        highlights.push({
          range: {
            start: { line: openingLine, character: openingIndex },
            end: {
              line: openingLine,
              character: openingIndex + openingKeyword.length,
            },
          },
          kind: DocumentHighlightKind.Read,
        });
