...
/**
 * Returns dynamic, fully localized pricing tiers based on active VS Code display language.
 */
...
async function backgroundRevalidate(
  ...
  try {
    ...
    if (res.status === "VALID") {
      ...
      const cust = (meta.customer as Record<string, unknown> | undefined) || {};
      const prod = (meta.product as Record<string, unknown> | undefined) || {};
      ...
      const updatedCache: LicenseCacheData = {
        ...
        valid: true,
        customerName: cust.name ? String(cust.name) : cache.customerName,
        customerEmail: cust.email ? String(cust.email) : cache.customerEmail,
        productName: prod.name ? String(prod.name) : cache.productName,
        subscriptionStatus: meta.status
          ? String(meta.status)
          : cache.subscriptionStatus,
        subscriptionEndsAt: meta.expires_at
          ? String(meta.expires_at)
          : cache.subscriptionEndsAt,
        activationLimit: Number(
          meta.activations_limit ||
            meta.activation_limit ||
            cache.activationLimit ||
            5,
        ),
        activationUsage: Number(
          meta.instances_count ||
            meta.activations_count ||
            cache.activationUsage ||
            1,
        ),
      };
      ...
      try {
        ...
        if (data && data.id && !data._httpError) {
          ...
          const cacheData: LicenseCacheData = {
            ...
            variantName: String(prod.name || "Pro Edition (Dodo Payments)"),
            ...
            subscriptionEndsAt: data.expires_at ? String(data.expires_at) : null,
            activationLimit: Number(
              data.activations_limit || data.activation_limit || 5,
            ),
            activationUsage: Number(
              data.instances_count || data.activations_count || 1,
            ),
          };
...
/**
 * Команда проверки статуса лицензии с живой онлайн-верификацией через Dodo Payments API.
 */
async function checkLicenseStatusCommand(context: vscode.ExtensionContext) {
  let cache = await loadLicenseCache(context);
  ...
  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: "Verifying license with Dodo Payments API...",
      cancellable: false,
    },
    async () => {
      try {
        ...
        if (res.status === "VALID") {
          ...
          const meta = (res.meta as Record<string, unknown> | undefined) || {};
          const cust =
            (meta.customer as Record<string, unknown> | undefined) || {};
          const prod =
            (meta.product as Record<string, unknown> | undefined) || {};
          ...
          cache = {
            ...
            subscriptionStatus: meta.status
              ? String(meta.status)
              : cache!.subscriptionStatus || "active",
            subscriptionEndsAt: meta.expires_at
              ? String(meta.expires_at)
              : cache!.subscriptionEndsAt || null,
            activationLimit: Number(
              meta.activations_limit ||
                meta.activation_limit ||
                cache!.activationLimit ||
          ...
          await saveLicenseCache(context, cache!);
          isPremiumCached = true;
        } else if (res.status === "REVOKED") {
          await clearLicenseCache(context);
          isPremiumCached = false;
        ...
        // Сетевая ошибка — используем локальный офлайн-кэш
      }

      if (isPremium()) {
        const now = Date.now();
        const offlineExpDate = new Date(cache!.expiresAt);
        const offlineDaysRemaining = Math.max(
          0,
          Math.ceil((offlineExpDate.getTime() - now) / (24 * 60 * 60 * 1000)),
        );

        let subText = "Lifetime Plan (Permanent Unlimited Access)";
        if (cache!.subscriptionEndsAt) {
          ...
          subText = `Active until ${subEnd.toLocaleDateString()} (${subDays}d remaining)`;
        }
