...
<p align="center">
  ...
  <a href="https://checkout.dodopayments.com/buy/pdt_0NmAUzwdbzeERSktsOLTp"><b>⚡ Pro Monthly ($9.99/мес)</b></a> • 
...
### 17. 📜 Декодер логов KUKA Event Log (.evt) (Pure-TS KRC Decoder)
*Анализ бинарных журналов событий KRC без внешних утилит и зависимостей.*
* **Нативный pure-TS декодер**: парсит файлы Windows EVTX контроллера (`KrcLogS.evt`, `KrcLogB.evt`, `KrcLogP.evt`, `KrcLogC.evt`, `KrcLogU.evt`) с нулевыми внешними зависимостями.
* **Зарегистрированный стандартный редактор**: двойной клик по любому `.evt` файлу в проводнике VS Code мгновенно открывает интерактивный журнал.
* **Каталог CrossMeld из 2,050+ сообщений**: автономный словарь сообщений на всех 6 языках (EN, DE, RU, ES, IT, TR) с динамической подстановкой технологических параметров `{0}`, `{1}`.
* **Фильтрация и переход к строке**: фильтрация по важности (`ERROR`, `WARNING`, `INFO`, `DIALOG`), источнику и времени, быстрый переход к строке файла программы `.src` в один клик и экспорт в CSV/JSON.

---

### 18. ⚡ Матрица I/O сигналов и детектор аппаратных коллизий PLC
*Полный аудит дискретных и аналоговых сигналов проекта.*
* **Глубокое сканирование**: анализ всех объявлений `SIGNAL` в `.src` и `.dat` файлах проекта (`$IN`, `$OUT`, `$ANIN`, `$ANOUT`).
* **Обнаружение коллизий**: автоматический расчёт битовых диапазонов (от одиночных флагов до групп байт) с предупреждением о наложении адресов до физического запуска.
* **Интерактивная таблица и CSV-экспорт**: фильтрация по типам, мгновенный переход к объявлению в коде и экспорт в CSV для спецификаций.

---

### 19. 📊 Длина траектории и интеллектуальный расчёт сварки
*Точный пространственный 3D-расчёт геометрии движений и технологических циклов.*
* **3D евклидова длина траектории**: суммарный метраж пространственных перемещений TCP в миллиметрах и метрах.
* **Сегментация команд движения**: разбивка движений по типам `PTP`, `LIN`, `CIRC`, `SPLINE`.
* **Анализ процесса сварки**: распознавание блоков `ARCON`/`ARCOFF`, изолированный подсчёт суммарной длины сварных швов и времени горения дуги по технологической скорости.

---

### 20. 🔍 Нативный Copilot-Style интерактивный просмотр диффов (`KrlReviewService`)
*Хирургическая проверка кода бок о бок перед сохранением любых изменений на диск.*
* **Никаких модальных окон**: все трансформации кода, конвертация в сплайны, рефакторинг фолдов и очистка открываются в нативном VS Code Monaco Diff Editor (`оригинал ↔ предлагаемое`).
* **Встроенная панель действий**: кнопки в заголовке редактора для мгновенного принятия или отклонения — `✅ Принять файл` (`Ctrl+Enter`), `❌ Отклонить` (`Esc`) или `Принять все` (`Ctrl+Shift+Enter`).
* **Многофайловый конвейер проекта**: интерактивный виджет в статус-баре (`$(diff) KRL Diff: Файл X/Y`), навигатор быстрого перехода QuickPick с сохранением правок буфера и защитой от состояний гонки.

---

### 21. 🦾 Кинематическая изоляция сплайнов KSS и валидация на реальных флотах
*Критически важное для безопасности обновление движений в строгом соответствии с кинематикой сплайнов KUKA KSS 8.3–8.7.*
* **Изоляция осевых движений SPTP**: движения по осям `SPTP` всегда генерируются как отдельные команды вне декартовых блоков `SPLINE...ENDSPLINE` для предотвращения кинематических остановок контроллера.
* **Строгий линтер сплайн-блоков**: помечает ошибочное использование `SPTP` внутри декартовых блоков `SPLINE` как ошибку компиляции, защищая от аварийного останова SmartPAD на реальном роботе.
* **Модернизация движений в 1 клик (`krl.convertLegacyToSpline`)**: безопасный перевод устаревших команд `PTP`, `LIN`, `CIRC` в современные `SPTP`, `SLIN`, `SCIRC` с автоматическим ограничением рывка `$SGEAR_JERK`.
* **Протестировано на 10,687 движениях реальных заказчиков**: 100% подтвержденная надежность на архивах 8 заводов (Parsan, Magna, Atlas Copco, Saint-Gobain, Osten, Teknorob) с 0 ложных срабатываний.

---

### 22. 🌓 100% Адаптивные темы и полная шестиязычная локализация
*Идеальная визуальная интеграция без единого захардкоженного цвета.*
* **Адаптивные CSS-переменные**: интерфейс построен на токенах темы VS Code (`--vscode-*`) и адаптирован под тёмные, светлые (`body.vscode-light`) и высококонтрастные (`body.vscode-high-contrast`) темы.
* **Переключатель 6 языков**: динамическое переключение языка интерфейса (RU, EN, DE, ES, IT, TR) прямо в шапке инструментов и через команду `krl.switchLanguage`.
...
| **Декодер логов KUKA Event Log (.evt)** | ❌ | **✅ Pro** | Pure-TS декодер EVTX, 2,050+ сообщений на 6 языках, переход к строке |
| **Матрица I/O сигналов и детектор коллизий** | ❌ | **✅ Pro** | Сканирование `$IN`/`$OUT`, обнаружение наложения битов и экспорт в CSV |
| **Длина траектории и статистика сварки** | ❌ | **✅ Pro** | 3D пространственная длина, метраж швов и время горения дуги |
| **Нативный интерактивный просмотр диффов AI** | ❌ | **✅ Pro** | Side-by-side инспекция в редакторе Monaco, пакетное принятие и переход |
| **Кинематическая изоляция сплайнов KSS** | ❌ | **✅ Pro** | Вынос SPTP из декартовых блоков, валидация на 10,687 реальных движениях |
...
| Тариф | Цена | Скидка / оплата | Условия лицензии | Оформление |
|:---|:---:|:---|:---|:---:|
...
| ⏱️ **Pro Месячный** | **$9.99** / мес | Оплата ежемесячно | Все 50 промышленных Pro-инструментов • 1 ПК • Поддержка KRC2/KRC3/KRC4/KRC5 | [Оформить Pro Monthly](https://checkout.dodopayments.com/buy/pdt_0NmAUzwdbzeERSktsOLTp) |
| 👑 **Pro Годовой** | **$79.00** / год | **Экономия 35%** (~$6.58/мес) | Все 50 Pro-инструментов • 1 ПК • 30-дневный офлайн-буфер • KRC2–KRC5 | [Оформить Pro на год](https://checkout.dodopayments.com/buy/pdt_0NmAV012KFHSjUMyDomJ6) |
| 🏆 **Pro Пожизненный** | **$699.00** | **Один платёж — навсегда** | Все 50 Pro-инструментов • 1 ПК (1 рабочая станция) • KRC2/KRC3/KRC4/KRC5 • Пожизненные обновления | [Купить пожизненный Pro](https://checkout.dodopayments.com/buy/pdt_0NmAcoqVCfuwQ6Xx7qyqr) |
| 🏢 **Team Edition** | **$299.00** / год | B2B — счёт и коммерческое предложение | Все 50 Pro-инструментов • 25 рабочих станций завода • Поддержка KRC2–KRC5 | [Купить Team Edition](https://checkout.dodopayments.com/buy/pdt_0NnLCdgD69GiXDLRJ0K5v) |
| 🏭 **Enterprise Site** | **$1,499.00** / год | Безлимит рабочих мест | Все 50 Pro-инструментов • Безлимитные рабочие станции • Брендированные отчёты | [Купить Enterprise](https://checkout.dodopayments.com/buy/pdt_0NnLCdkLwd0dECkpSE1JP) |
...
<div align="center" style="margin: 25px 0;">
  <a href="https://checkout.dodopayments.com/buy/pdt_0NmAV012KFHSjUMyDomJ6" style="text-decoration:none;">
    <kbd style="font-size: 1.25em; padding: 14px 32px; background-color: #FF6600; color: white; border-radius: 10px; font-weight: bold; border: 1px solid #d15500; cursor: pointer; box-shadow: 0 4px 18px rgba(255,102,0,0.4);">
      ⚡ Выбрать тариф и оформить подписку (Dodo Checkout)
    </kbd>
  </a>
...
### 💡 Как выбрать подходящий тариф? (Экономика лицензирования):

* 💻 **Индивидуальные лицензии (Pro Monthly / Annual — 1 ПК):**
  * Лицензия на **1 рабочий ПК / ноутбук инженера**.
  * Полная промышленная поддержка поколений **KRC2 / KRC3 / KRC4 / KRC5 (KSS 5.x–8.7+)**.
* 🏆 **Pro Lifetime ($699 разово — 1 рабочая станция на 1 ПК):**
  * Постоянная лицензия на **1 ПК / ноутбук** без подписок и регулярных платежей с пожизненными обновлениями.
  * Любая предотвращённая авария робота или сэкономленный день пусконаладки окупает эту стоимость мгновенно.
* 🏢 **Тариф на 25 рабочих станций (Team Edition — $299/год):**
  * Пакет из **25 активаций рабочих станций** для отдела автоматизации и роботизации завода.
  * Централизованная оплата по B2B-счёту, полная поддержка KRC2–KRC5, закрывающие акты и единый корпоративный стандарт качества KRL-кода на всех проектах.
* 🏭 **Enterprise для завода (Enterprise Site — $1,499/год):**
  * *Почему годовая лицензия $1,499/год, если бессрочный тариф на 1 ПК стоит $699?*
  * Потому что это **БЕЗЛИМИТНЫЕ АКТИВАЦИИ НА ВЕСЬ ЗАВОД (Unlimited Workstations)**! Лицензия покрывает не 1 компьютер, а **все компьютеры, стенды и пульты предприятия (20, 50 или 100+ наладчиков, операторов и ремонтников)**. Для команды из 15 специалистов это всего **~$8 в месяц на рабочее место**!
  * **Прямая инженерная поддержка:** приоритетная связь напрямую с разработчиком через встроенный Telegram-шлюз поддержки.
  * **KUKA Fleet Backup Hub:** автоматизированная централизованная архивация, версионирование и diff-анализ бэкапов сотен роботов завода.
  * **Брендированные протоколы приёмки:** генерация официальных паспортов роботов и аудитов безопасности с логотипом заказчика (Ford, Renault, Saint-Gobain, TOGG).
  * **B2B-договор и закрывающие документы:** безналичный расчёт, счёт-фактура с НДС, акты приёма-передачи прав.
